...
export default function ResumeMatcher({ onSwitchToPrep }: Props) {
  ...
  const handleDrop = (e: React.DragEvent, target: 'resume' | 'jd') => {
    e.preventDefault();
    if (target === 'resume') setIsDraggingResume(false);
  ...
  // ── 分类卡片 ──
  const CategoryCard = ({ title, items, color, icon }: { title: string; items: string[]; color: string; icon: string }) => {
    if (!items || items.length === 0) return null;
    return (
      <div style={{
        background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 18,
  ...
  // ── 文件拖拽区 ──
  const DropZone = ({ target, label, value, isDragging, setDragging, fileRef }: {
    target: 'resume' | 'jd';
    label: string;
    value: string;
    isDragging: boolean;
    ...
    <div>
      ...
      >
        ...
        <input type="file" ref={fileRef as React.RefObject<HTMLInputElement>} hidden
          onChange={(e) => { const f = e.target.files?.[0]; if (f) processFile(f, target); }}
          accept=".pdf,.docx,.doc,.jpg,.jpeg,.png,.gif,.bmp,.webp,.txt,.md"
        />
      </div>
      <textarea
        value={value}
        onChange={(e) => target === 'resume' ? setResumeText(e.target.value) : setJdText(e.target.value)}
        placeholder={target === 'resume' ? '或在此粘贴简历内容...' : '或在此粘贴JD内容...'}
        style={{
  ...
  if (stage === 'result' && result) {
    return (
      <div style={{ maxWidth: 860, margin: '0 auto', padding: 24 }}>
        {/* 顶部评分区 */}
        <div style={{
          background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 28,
        ...
        {result.requirement_matching && result.requirement_matching.length > 0 && (
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: CYAN, marginBottom: 14 }}>
              📋 JD 逐条匹配分析
            </div>
            {result.requirement_matching.map((item, i) => (
              <div key={i} style={{
                padding: '10px 0', borderBottom: i < result.requirement_matching.length - 1 ? `1px solid ${BORDER}` : 'none',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{
                    fontSize: 11, padding: '2px 8px', borderRadius: 4, fontWeight: 600,
                    background: item.match_level.includes('强') ? 'rgba(34,255,34,0.1)' :
                               item.match_level.includes('中') ? 'rgba(255,255,0,0.1)' :
                               'rgba(255,68,68,0.1)',
                    color: item.match_level.includes('强') ? GREEN :
                           item.match_level.includes('中') ? YELLOW : RED,
                  }}>
                    {item.match_level}
                  </span>
                ...
                {item.resume_evidence && (
                  <div style={{ fontSize: 12, color: DIM, marginLeft: 4 }}>
                    📄 简历证据：{item.resume_evidence}
                  </div>
                )}
                <div style={{ fontSize: 12, marginLeft: 4, marginTop: 2 }}>
                  <span style={{ color: YELLOW }}>⚠️ {item.gap_risk}</span>
                </div>
                <div style={{ fontSize: 12, marginLeft: 4 }}>
                  <span style={{ color: CYAN }}>💡 面试策略：{item.strategy}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {result.prep_suggestions && result.prep_suggestions.length > 0 && (
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: MAGENTA, marginBottom: 12 }}>
              🎯 面试备考建议
            </div>
            {result.prep_suggestions.map((s, i) => (
              <div key={i} style={{ fontSize: 13, color: '#aaa', marginBottom: 8, paddingLeft: 12, borderLeft: `2px solid ${MAGENTA}` }}>
                {i + 1}. {s}
              </div>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => { setStage('input'); setResult(null); }}
            style={{
              padding: '10px 24px', background: 'transparent', color: DIM, border: `1px solid ${BORDER}`,
              borderRadius: 8, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit',
            }}>
            ← 重新匹配
          </button>
          <button onClick={() => onSwitchToPrep(resumeText, jdText)}
            style={{
              padding: '10px 24px', background: CYAN, color: '#000', border: 'none',
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: 24 }}>
      {error && (
        <div style={{
          background: 'rgba(255,68,68,0.1)', border: `1px solid ${RED}`, borderRadius: 8,
          padding: 10, marginBottom: 16, fontSize: 13, color: RED,
        }}>
          ⚠️ {error}
          <button onClick={() => setError('')} style={{
            marginLeft: 12, background: 'none', border: 'none', color: RED, cursor: 'pointer', fontSize: 16,
          }}>×</button>
        </div>
      )}

      <DropZone target="resume" label="📄 简历" value={resumeText} isDragging={isDraggingResume} setDragging={setIsDraggingResume} fileRef={resumeFileRef} />

      <div style={{ height: 16 }} />

      <DropZone target="jd" label="📋 岗位描述 (JD)" value={jdText} isDragging={isDraggingJd} setDragging={setIsDraggingJd} fileRef={jdFileRef} />

      <div style={{ marginTop: 20, textAlign: 'center' }}>
        <button onClick={handleMatch}
          style={{
            padding: '12px 40px', background: CYAN, color: '#000', border: 'none',
            borderRadius: 10, cursor: 'pointer', fontSize: 15, fontWeight: 700, fontFamily: 'inherit',
          }}>
          🔍 开始匹配
        </button>
